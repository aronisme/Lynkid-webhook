...
amounts.forEach(amount => {
    ...
    patterns.forEach(dataToSign => {
        ...
        if (s1 === expectedSig) {
             console.log(`FOUND SHA256! Formula matches: merchantKey + ${dataToSign} (amount used: ${amount})`);
             found = true;
        }
        if (s2 === expectedSig) {
             console.log(`FOUND SHA256! Formula matches: ${dataToSign} + merchantKey (amount used: ${amount})`);
             found = true;
        }
    });
});
